...
import { Code, Database, Server, Cloud, X, Info } from 'lucide-react'
...
const CapabilityStage3 = () => {
  ...
  const codingTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  const pulseTimeoutRef = useRef<NodeJS.Timeout | null>(null)

  // Микро-анимация кодинга
  useEffect(() => {
    const startCoding = () => {
    ...
    startCoding()
    const interval = setInterval(startCoding, 5000)
  ...
  const content = {
    ...
    subtitle: "Масштабируемые серверные решения",
    features: [
      { 
        ...
        icon: Code,
        color: "blue",
        details: "RESTful API, GraphQL, WebSocket, gRPC, автоматическая документация, версионирование, rate limiting"
      },
      { 
        ...
        icon: Database,
        color: "green",
        details: "Схемы БД, индексы, оптимизация запросов, репликация, шардинг, миграции, backup стратегии"
      },
      { 
        title: "Microservices", 
        description: "Docker, Kubernetes, Istio",
        icon: Server,
        color: "purple",
        details: "Контейнеризация, оркестрация, service mesh, мониторинг, логирование, tracing, circuit breakers"
      },
      { 
        ...
        icon: Cloud,
        color: "orange",
        details: "Auto-scaling, load balancing, CDN, serverless, managed services, security, compliance"
      }
    ],
    benefits: [
    ...
    timeline: "4-8 недель",
    guarantee: "Гарантируем высокопроизводительный backend"
  }
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          ...
          >
            ...
            <p className="text-xs text-white/70">Node.js, Python, Go...</p>
          </motion.div>
        </div>
