...
import { Users, MessageCircle, Headphones, Clock, X, Info } from 'lucide-react'
...
const CapabilityStage8 = () => {
  ...
  const supportingTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  const pulseTimeoutRef = useRef<NodeJS.Timeout | null>(null)

  // Микро-анимация поддержки
  useEffect(() => {
    const startSupporting = () => {
      if (prefersReducedMotion) return
      
      setIsSupporting(true)
      
      if (supportingTimeoutRef.current) {
        clearTimeout(supportingTimeoutRef.current)
      }
      
      supportingTimeoutRef.current = setTimeout(() => {
        setIsSupporting(false)
      }, 2000)
    }

    startSupporting()
    const interval = setInterval(startSupporting, 5000)
  ...
  const content = {
    ...
    subtitle: "Техническая поддержка и сопровождение",
    features: [
      { 
        title: "24/7 Support", 
        ...
        icon: Users,
        color: "blue",
        details: "24/7 техническая поддержка, мониторинг системы, быстрая реакция на инциденты, SLA гарантии"
      },
      { 
        title: "Live Chat", 
        description: "Онлайн чат поддержка",
        icon: MessageCircle,
        color: "green",
        details: "Мгновенная помощь через чат, интеграция с системой тикетов, автоматические ответы, эскалация"
      },
      { 
        title: "Phone Support", 
        description: "Телефонная поддержка",
        icon: Headphones,
        color: "purple",
        details: "Прямая связь с экспертами, удаленное управление, screen sharing, запись звонков для анализа"
      },
      { 
        title: "SLA Guarantee", 
        description: "Гарантии SLA",
        icon: Clock,
        color: "orange",
        details: "99.9% uptime гарантия, время реакции < 15 мин, время решения < 4 часа, компенсации за нарушения"
      }
    ],
    benefits: [
    ...
    technologies: [
      ...
      "AWS Support, Azure Support, GCP Support"
    ],
    timeline: "Постоянно",
    guarantee: "Гарантируем надежную поддержку"
  }
    ...
    <div className="relative w-full h-full flex items-center justify-center p-2">
      ...
      >
        ...
        <div className="text-center mb-4">
          <motion.div
            className="inline-flex items-center justify-center w-10 h-10 mb-2 rounded-full bg-gradient-to-br from-orange-400 to-red-500"
            animate={isSupporting ? {
              scale: [1, 1.1, 1],
              rotate: [0, 5, -5, 0]
            } : {}}
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          ...
          >
            ...
            <p className="text-xs text-white/70">Zendesk, Slack, Jira...</p>
          </motion.div>
        </div>
