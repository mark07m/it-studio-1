    ...
    <main className="w-full h-full flex items-center justify-center p-4 pt-24 sm:pt-28 pb-8 lg:pb-12 flex-1" role="main" aria-label="Cloud Infrastructure section">
      <div className="max-w-6xl w-full flex flex-col px-8 sm:px-12 md:px-16 lg:px-20">
        <header className="text-center mb-8">
          ...
          >
            Инфраструктура как код. Автоматизация, мониторинг, безопасность — всё на уровне AWS.
          </motion.p>
        </header>
        
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" aria-label="Cloud features">
          {[
            { title: 'Infrastructure as Code', description: 'Terraform, Ansible, автоматизация', icon: '🏗️' },
            { title: 'Kubernetes', description: 'Оркестрация контейнеров', icon: '☸️' },
            { title: 'Monitoring', description: 'Мониторинг и алерты 24/7', icon: '📊' }
          ].map((feature, index) => (
            <motion.article
              key={feature.title}
        ...
        {/* Анимированная схема Kubernetes кластера */}
        <motion.div
